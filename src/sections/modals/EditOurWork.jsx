...
import { updateOurWorkData } from '../../services/api';

export default function EditOurWork({ item, onClose, onRefresh }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateOurWorkData(item.id, dataToSend);
    ...
    <div className="modal-overlay p-4">
      <div className="modal-panel">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[calc(100vh-12rem)] overflow-y-auto">
          ...
          {/* Row 3: Description (Textarea transformation for better UX) */}
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
              Description *
            </label>
          ...
          {/* Row 4: Image Upload Visual Upgrade */}
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
               Image
            </label>
