...
import { storeTopCategoriesData } from "../../services/api";

export default function AddStats({ isOpen, onClose, onAdd }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await storeTopCategoriesData(formData);
    ...
    <div className="modal-overlay no-scrollbar">
      <div className="modal-panel">
        ...
        <div className="modal-header">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Add New Categories</h2>
            <p className="text-muted-foreground text-sm mt-1">Create a new menu item</p>
          </div>
          <button
            onClick={onClose}
            className="icon-btn"
