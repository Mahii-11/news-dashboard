...
import { deleteTopCategoriesData, getTopCategoriesData } from '../services/api'; 
...
export default  function Categories() {
  ...
  const loadStudyStepData = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getTopCategoriesData();
...
const handleDelete = async (id) => {
  ...
  try {
    await deleteTopCategoriesData(id); 
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-title text-4xl">Top Categories</h1>
          <p className="page-subtitle">Create, edit, and organize top navigation bar items</p>
        </div>
        <button
